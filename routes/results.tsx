...
import { MapIsland } from "../islands/MapIsland.tsx"; // Adjust the import path as necessary
...
export const handler: Handlers<PlantData[]> = {
  GET(req, ctx) {
    ...
    try {
      const results: PlantData[] = JSON.parse(decodeURIComponent(atob(data)));
  ...
  return (
    <>
            ...
            <div className="text-center">
              ...
              <div className="grid place-items-center">
                <p className="select-text max-w-lg py-6 break-words text-green-900 font-medium text-sm md:text-md md:font-bold tracking-tight">
                  Welcome to your <strong>personalized</strong> plant recommendations, powered by{" "}
                  <strong>AI</strong>! By growing these plants, you're not only enhancing your surroundings but also{" "}
                  <strong>contributing</strong> to the well-being of our planet. Remember, plants are our{" "}
                  <strong>best friends</strong>. These <strong>recommendations</strong> are tailored to your location
                  and local weather, so you can nurture these green companions with confidence. Let's{" "}
                  <strong>grow together</strong> and make a <strong>positive</strong> impact on Earth! Scroll Down {":)"}
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="my-10 grid place-items-center">
        <MapIsland latitude={latitude} longitude={longitude} />
      </div>
          ...
          {results.map((x: PlantData) => (
            <div className="w-full hover:shadow-2xl md:hover:scale-105 transition card py-2 md:w-96 select-none bg-base-300 shadow-xl" key={x.name}>
              ...
              <div className="card-body">
                <h2 className="card-title underline underline-offset-8 select-text">{x.name}</h2>
                <h3 className="tracking-tight font-5xl font-bold mt-4 border-white border w-fit rounded-md px-3 py-1">
                  ⏰ {x.growth_time}
                </h3>
                <p className="mt-2 select-all mb-4 opacity-90 font-semibold tracking-wide break-words">{x.note}</p>
